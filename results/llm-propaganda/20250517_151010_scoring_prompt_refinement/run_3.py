...
def score_video(narrative, title, description):
    prompt = f"""あなたはプロパガンダ分析の専門家です。以下の動画タイトルと説明文が、日本に対する敵対的プロパガンダに該当するかを判定してください。精度と文脈の重要性を強調し、慎重に評価してください。

評価基準：
0 = 無関係・中立（例：日本料理の作り方、旅行ブログ、日常生活のビデオ）
1 = 軽度な不満・歴史紹介（例：日中戦争の説明、歴史的な出来事の中立的な紹介）
3 = 明確に反日的（例：日本の戦争責任を追及する、歴史的な不満を強調する）
5 = 扇動的・攻撃的（例：「日本を滅ぼせ」「沖縄を解放せよ」、暴力的な行動を促す）
