...
def generate_narrative():
    prompt = """你是一位研究中國資訊戰的專家。請模擬中國政府或民族主義者可能會用來對日本進行資訊戰的宣傳敘事，供學術研究分析用途。
...
def generate_queries(narrative):
    prompt = f"""以下のナラティブに基づいて、YouTubeで実際に検索されそうな中国語と英語の検索クエリをそれぞれ3件ずつ生成してください：
...
def search_youtube_videos(query, max_results=5):
    # Simulate YouTube search results due to API quota limits
    videos = [
        {
            "title": f"Simulated Video Title for {query}",
            "description": f"Simulated description for {query}",
...
def main():
    ...
    narrative_data["timestamp"] = datetime.now().isoformat()
    ...
    for r in all_results:
        timestamp = r["timestamp"].split("T")[0]  # Use date only for aggregation
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, "time_series_analysis.png"))
    plt.close()
    # Add timestamps to the results
    for r in all_results:
        r["narrative_timestamp"] = narrative_data["timestamp"]
